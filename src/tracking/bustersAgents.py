...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghost in livingGhostPositionDistributions:
            ...
            for k, v in ghost.items():
                if v > prob_ghost:
                    prob_ghost = v
                    pos_ghost = k
            ghost_info.append(pos_ghost)

        i = 0
        distance = float('inf')
        choose_pos = (0, 0)
        while i < len(ghost_info):
            now_dis = self.distancer.getDistance(pacmanPosition, ghost_info[i])
            if now_dis < distance:
                distance = now_dis
                choose_pos = ghost_info[i]
            i += 1

        final_action = []
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            new_dis = self.distancer.getDistance(choose_pos, successorPosition)
            if new_dis < distance:
        ...
        if not final_action:
            ...
            return random.choice(final_action)
        "*** END YOUR CODE HERE ***"
