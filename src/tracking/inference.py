...
import hunters
from util import manhattanDistance, raiseNotDefined, Counter, sample
...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[GHOST0] = [(x, y) for x in range(0, X_RANGE) for y in range(0, Y_RANGE)]
    variableDomainsDict[OBS0] = [x for x in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict[PAC] = [(x, y) for x in range(0, X_RANGE) for y in range(0, Y_RANGE)]
    variableDomainsDict[OBS1] = [x for x in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict[GHOST1] = [(x, y) for x in range(0, X_RANGE) for y in range(0, Y_RANGE)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for value in eliminationOrder:
            currentfactorslist, joinedfactor = joinFactorsByVariable(currentfactorslist, value)
            if len(joinedfactor.unconditionedVariables()) != 1:
                eliminated_factor = eliminate(joinedfactor, value)
                currentfactorslist.append(eliminated_factor)

        final_joint = joinFactors(currentfactorslist)

        return normalize(final_joint)
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            observationProb = self.getObservationProb(observation, pac_pos, pos, jail_pos)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for k, v in newPosDist.items():
                newBeliefs[k] += v * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_of_positions = len(self.legalPositions)
        for i in range(num):
            pos = self.legalPositions[i % num_of_positions]
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.legalPositions:
            BeliefDistribution[p] = self.particles.count(p)
