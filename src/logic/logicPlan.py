...
def sentence2():
    ...
    third = ~(B & ~C) >> A
    forth = ~D >> C
    return conjoin(first, second, third, forth)
    "*** END YOUR CODE HERE ***"
...
def sentence3():
    ...
    first = B % disjoin(A & ~D, ~A & C)
    second = ~(A & C)
    return conjoin(first, second, C)
    "*** END YOUR CODE HERE ***"
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    for x, y in all_coords:
        s_a = PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, t)
        pacphysics_sentences.append(s_a)

    non_walls = []
    for x, y in non_outer_wall_coords:
        non_walls.append(PropSymbolExpr(pacman_str, x, y, t))
    pacphysics_sentences.append(exactlyOne(non_walls))

    directions = []
    for action in DIRECTIONS:
        directions.append(PropSymbolExpr(action, t))
...
def check_location_satisfiability(x1_y1, x0_y0, action0, action1, problem):
    ...
    m1 = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x1, y1, 1))
    m2 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, 1))
    return m1, m2
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem):
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            non_walls.append(PropSymbolExpr(pacman_str, x, y, t))
        KB.append(exactlyOne(non_walls))
...
def foodLogicPlan(problem):
    ...
    for t in range(50):
        ...
        all_food = []
        for x, y in food:
            all_food.append(~PropSymbolExpr(food_str, x, y, t))
        food_goal = conjoin(all_food)
        ...
        for x, y in food:
            exp = PropSymbolExpr(food_str, x, y, t) & PropSymbolExpr(pacman_str, x, y, t)
            exp_new = ~PropSymbolExpr(food_str, x, y, t+1)
            KB.append(exp_new % exp | ~PropSymbolExpr(food_str, x, y, t))
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            exp_a = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x, y, t))
            exp_b = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x, y, t))
            if exp_b:
        ...
        possible_locs_by_timestep.append(possible_locations)
...
def mapping(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            exp_a = findModel(conjoin(KB) & ~PropSymbolExpr(wall_str, x, y))
            exp_b = findModel(conjoin(KB) & PropSymbolExpr(wall_str, x, y))
        ...
        # 这里需要转换一下数组的形式
        now_wall_grid = [[] for item in range(len(known_map))]
        for i in range(0, len(known_map)):
            for j in range(0, len(known_map[i])):
                val = known_map[i][j]
                if val == -1:
                    now_wall_grid[i].append(0)
                else:
                    now_wall_grid[i].append(val)

        KB.append(allLegalSuccessorAxioms(t + 1, now_wall_grid, non_outer_wall_coords))
...
def slam(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            exp_a = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x, y, t))
            exp_b = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x, y, t))
            exp_c = findModel(conjoin(KB) & ~PropSymbolExpr(wall_str, x, y))
            exp_d = findModel(conjoin(KB) & PropSymbolExpr(wall_str, x, y))
        ...
        for i in range(0, len(known_map)):
            for j in range(0, len(known_map[i])):
                ...
                if val == -1:
                    ...
                    now_wall_grid[i].append(val)
        KB.append(SLAMSuccessorAxioms(t + 1, now_wall_grid, non_outer_wall_coords))
    "*** END YOUR CODE HERE ***"
